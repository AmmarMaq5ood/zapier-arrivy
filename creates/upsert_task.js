...
const upsertTask = (z, bundle) => {
  return tasks_util.upsert_task(z, bundle, bundle.inputData.unscheduled);
};
...
module.exports = {
  ...
  display: {
    label: 'Create or Update an Arrivy Task',
    description: 'Creates or Updates an Arrivy Task. This action uses the provided external_id to decide whether it\'s create or an update operation. If the task date/time isn\'t yet available please set unscheduled option to true.'
  },

  operation: {
    inputFields: [
      {key: 'external_id', label:'External Id of the task from another system. Please ensure it\'s a unique value for each task.', required: true, type: 'string'},
      ...
      {key: 'unscheduled', label:'Unscheduled', required: false, type: 'boolean'},
